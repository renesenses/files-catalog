...
#	print Dumper(@file_recs);
	$schema->populate('file', [ [qw/file_md5 file_size file_type file_nbocc file_time/], @file_recs ]);  
...
#	print Dumper(@loc_recs);	
	$schema->populate('location', [ [qw/ file_md5  loc_full loc_name loc_vol loc_path loc_ext/], @loc_recs ]); 		
} 
else {
	die;
}
